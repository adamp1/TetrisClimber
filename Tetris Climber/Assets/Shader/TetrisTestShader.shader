    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.uv0 = v.texcoord0;
                float4 node_1686_k = float4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);
                float4 node_1686_p = lerp(float4(float4(mul(unity_ObjectToWorld, v.vertex).rgb,0.0).zy, node_1686_k.wz), float4(float4(mul(unity_ObjectToWorld, v.vertex).rgb,0.0).yz, node_1686_k.xy), step(float4(mul(unity_ObjectToWorld, v.vertex).rgb,0.0).z, float4(mul(unity_ObjectToWorld, v.vertex).rgb,0.0).y));
                float4 node_1686_q = lerp(float4(node_1686_p.xyw, float4(mul(unity_ObjectToWorld, v.vertex).rgb,0.0).x), float4(float4(mul(unity_ObjectToWorld, v.vertex).rgb,0.0).x, node_1686_p.yzx), step(node_1686_p.x, float4(mul(unity_ObjectToWorld, v.vertex).rgb,0.0).x));
                float node_1686_d = node_1686_q.x - min(node_1686_q.w, node_1686_q.y);
                float node_1686_e = 1.0e-10;
                float3 node_1686 = float3(abs(node_1686_q.z + (node_1686_q.w - node_1686_q.y) / (6.0 * node_1686_d + node_1686_e)), node_1686_d / (node_1686_q.x + node_1686_e), node_1686_q.x);;
                float node_6286 = (_boom2+_boom);
                v.vertex.xyz += lerp(float3(0,0,0),(((float3(float2(node_1686.r,node_1686.g),node_1686.b)*0.2)*_offset)+0.0),node_6286);
